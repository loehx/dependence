 ...
 *
 */
var Dependence = module.exports = function() {
	this.dependencies = {};
	this.shared = {};
	this.providers = [];
...
Dependence.prototype = {
	...
	register: function(name, dependency) {
		if (typeof dependency !== 'function')
			return this.shared[name] = dependency;
	...
	resolveFunction: function(func, callback, history) {
		...
		if (func.__dependencies)
			return callFunc(func.__dependencies);

		// Extract dependency names
		var dependencyNames = helpers.extractFunctionParameters(func);
		...
		async.map(dependencyNames, function(name, callback) {
			...
			callFunc(func.__dependencies = dependencies);
		});
	...
	resolveDependency: function(name, callback, history) {
		...
		if (history.indexOf(name) !== -1) {
			console.error("Circular dependency found! This was already resolved: " + name);
		...
		async.each(self.providers, function(provider, callback) {
		...
		function done(dependency) {
			...
			self.resolveFunction(dependency, function(resolved) {
				// console.log(' #### dependency.shared', dependency);

				if (dependency.shared !== false)
